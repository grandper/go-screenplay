...
func TestSeeAllOfAction(t *testing.T) {
	t.Parallel()
	...
	t.Run("implements the stringer interface", func(t *testing.T) {
		...
		assert.Equal(
			t,
			"see if the profil widget is containing the text Adam, and see if the login form is containing the text adam@google.com",
			action.String(),
		)
	})
}
