...
func TestPauseAction(t *testing.T) {
	...
	t.Run("pauses for a given time", func(t *testing.T) {
		start := time.Now()
	...
	t.Run("fails if no reason is provided for the pause", func(t *testing.T) {
	...
	t.Run("implements the stringer interface", func(t *testing.T) {
		...
		assert.Equal(t, "pause for 1 second because it is a test", action1.String())

		action2 := action.PauseFor(1).Seconds().Because("it is a test")
		assert.Equal(t, "pause for 1 second because it is a test", action2.String())

		action3 := action.PauseFor(20).Milliseconds().Because("it is a test")
		assert.Equal(t, "pause for 20 milliseconds because it is a test", action3.String())

		action4 := action.PauseFor(20).Millisecond().Because("it is a test")
		assert.Equal(t, "pause for 20 milliseconds because it is a test", action4.String())
	})
}
